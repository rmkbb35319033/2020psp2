...
主に修正したものは、①関数のreturnに入れる式と②whileの中身である。
①については、普通に平均を出す際に、総和が大きくなりすぎてしまうことを防ぐため、式をうまく変形したものを使った。
②については、whileの中身で①で作った関数を使う順番に注意しながら使った。
AVE（平均）の中にave_online()で計算したものを返すことにより、うまくループ的な処理を再現した。
